...
export function extractLabels(inputString) {
  ...
  for (let token of lexer) {
    switch (token.type) {
      case "whitespace":
      case "text":
        texts.push({ text: token.value, isLabel: false });
        break;
      case "name":
        texts.push({ text: token.value, isLabel: true });
        labels.push({ name: token.value, args: [] });
        break;
      case "callEmpty":
        texts.push({ text: token.value, isLabel: true });
        labels.push({ name: token.value, args: [] });
      ...
      case "call":
        texts.push({ text: token.value, isLabel: true });
        labels.push({ name: token.value, args: collectParams(lexer) });
...
function createLexer() {
  // All possible (unicode) identifiers in JavaScript (https://mathiasbynens.be/demo/javascript-identifier-regex)
